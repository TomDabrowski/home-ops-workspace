...
test("accepts wealth snapshots in both old and newer saved shapes", () => {
  ...
  const snapshots = payload as Array<{ anchorMonthKey?: string; monthlyStatus?: { basisInvestmentState?: string } }>;
  assert.equal(snapshots[1].anchorMonthKey, "2026-04");
  assert.equal(snapshots[1].monthlyStatus?.basisInvestmentState, "pending_cash");
});
