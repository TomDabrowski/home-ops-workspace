...
function renderMonthSourceStats(review) {
  renderRows("monthReviewSourceStats", [
    ...
    ["Freie Einnahmen im Monat", euro.format(review.row.importedIncomeAvailableAmount)],
    ["Alle Monatsausgaben", euro.format(review.row.importedExpenseAmount)],
...
function buildMusicYearData(importDraft, monthlyPlan, selectedMonthKey) {
  ...
  const rows = monthKeys.map((monthKey) => {
    ...
    return {
      ...
      prepaid,
      estimatedTax,
      balance: roundCurrency(prepaid - estimatedTax),
    };
  });
...
function renderMusicWorkspace(importDraft, monthlyPlan, monthKey) {
  ...
  if (summary) {
    const taxReason =
      data.yearlyMusicGross > 0
        ...
        : `Herleitung: Sobald im Jahr ${data.selectedYear} Musik-Umsatz vorliegt, berechnet die App hier den effektiven Zusatz-Steuersatz aus der Differenz zwischen Steuer mit und ohne Musik-Gewinn.`;
    const entries = [
      ["Musik brutto Jahr", euro.format(data.yearlyMusicGross)],
      ["Musik-Ausgaben Jahr", euro.format(data.yearlyMusicExpenses)],
      ["Musik netto vor Steuer", euro.format(data.yearlyProfit)],
      ["Steuersatz auf Musik", formatPercent(data.effectiveRate)],
      ["Steuer geschätzt", euro.format(data.estimatedTaxAnnual)],
      ["Vorausgezahlt", euro.format(data.yearlyPrepaid)],
      ["Voraussichtlicher Saldo", euro.format(data.yearlyBalance)],
      ["Begründung", taxReason],
    ];
  ...
  renderRows("musicMonthRows", data.rows, (row) => `
    <tr>
      ...
      <td>${euro.format(row.prepaid)}</td>
      <td>${makeMoneyCell(row.balance)}</td>
    </tr>
  `);
...
function renderMonthReview(importDraft, monthlyPlan, monthKey) {
  ...
  if (summary) {
    const entries = [
      ...
      ["Variable Ausgaben", euro.format(review.row.importedExpenseAmount)],
      ["Monatssaldo", euro.format(review.row.netAfterImportedFlows)],
      [
        "Erwarteter Cash-Kontostand Monatsende",
        review.row.safetyBucketEndAmount !== undefined ? euro.format(review.row.safetyBucketEndAmount) : "-",
      ],
      [
        "Erwarteter Investment-Stand Monatsende",
      ...
      [
        "Cash-Zufluss im Monat",
        euro.format((review.row.salaryAllocationToSafetyAmount ?? 0) + (review.row.musicAllocationToSafetyAmount ?? 0)),
      ],
      [
        "Sparplan ins Investment",
        euro.format(review.row.salaryAllocationToInvestmentAmount ?? 0),
      ],
    ];
    if (review.row.wealthAnchorApplied) {
      entries.push([
        "Workbook-Vergleich Cash Monatsende",
        review.row.safetyBucketAnchorAmount !== undefined ? euro.format(review.row.safetyBucketAnchorAmount) : "-",
      ]);
      entries.push([
        "Workbook-Vergleich Investment Monatsende",
        review.row.investmentBucketAnchorAmount !== undefined ? euro.format(review.row.investmentBucketAnchorAmount) : "-",
      ]);
      entries.push([
        "Workbook-Vergleich Gesamtvermögen Ende",
        review.row.projectedWealthAnchorAmount !== undefined ? euro.format(review.row.projectedWealthAnchorAmount) : "-",
      ]);
    }
...
function renderWealthSnapshotPlanner(importDraft) {
  ...
  const fallbackDate = `${reviewFocusMonthKey}-01`;
  ...
  saveButton.onclick = async () => {
    ...
    await refreshFinanceView({
      ...
      detail: `${statusDetailForMode(result.mode)} Wirkt für ${snapshotDate.slice(0, 7)} als Monatsanker.`,
      tone: result.mode === "project" ? "success" : "warn",
    });
  };
}
